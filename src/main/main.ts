...
import { NetworkStreamer } from "../shared/NetworkStreamer";
...
app.whenReady().then(() => {
  ...
  const net = express();
  let networkStreamer = new NetworkStreamer(net);
  const port = 3000;
  net.listen(port, () => console.log(`Server running on http://localhost:${port}`));

  // create mjpeg streamer ipc
  ipcMain.on("send-frame", (event, name: string, imageBuffer: string) => {
    // console.log("Sending frame");
    // console.log(imageBuffer);
    const buffer = Buffer.from(imageBuffer, "base64");
    networkStreamer.sendFrame(name, buffer);
  });

  ipcMain.on("update-network-names", (event, names: Array<string>) => {
    networkStreamer.setupStreamRoutes(names);
  });

  ipcMain.on("clear-network-streams", (event) => {
    networkStreamer.clearNetworkStreams();
  });

  ipcMain.on("log", (event, details: string) => {
    console.log(details);
  });
